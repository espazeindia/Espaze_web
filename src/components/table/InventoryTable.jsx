...
function InventoryTable({
  ...
  loading,
  setReload,
  userRole, // pass karo parent se user role
}) {
  const { theme } = useMode();
  ...
  const handleUpdateInventory = async (data) => {
    try {
      ...
      if (res.success) {
        setProducts((prev) =>
          prev.map((p) =>
            p.id === data.id ? { ...p, visible: !p.visible } : p
          )
        );
  ...
  // ✅ Ensure table renders only after products loaded
  if (loading) {
    return (
      <div className="overflow-x-auto">
        <div className={`min-w-[120vw] ${theme ? "bg-white" : "bg-zinc-800"} animate-pulse`}>
        ...
        <div className="min-w-[120vw]">
          ...
          <div className="grid grid-cols-[2fr_5fr_3fr_1fr_4fr_3fr_6fr_3fr_2fr_3fr_2fr_3fr_2fr_2fr] border-b py-4 text-sm border-gray-300 border-dotted">
            {[
              ...
              "Product Description", "Manufacturing Date", "Quantity", "Expiry Date",
              "Prices", "Status", "Visible", "Actions"
            ].map((header,i)=>(
              <div key={i} className={`text-center font-semibold ${theme ? "text-[#4110a2]" : "text-[#b898fa]"}`}>{header}</div>
            ))}
          </div>

          {/* Table Body */}
          <div className="h-[50vh] overflow-y-scroll sideBarNone">
            {products.length === 0 ? (
                ...
                >
                  ...
                  <div className="flex items-center justify-center" onClick={(e)=>e.stopPropagation()}>
                    <Switch
                      ...
                      sx={theme ? {
                        ...
                        "--Switch-trackHeight":"19px",
                        "--Switch-thumbSize":"10px",
                        "&.Mui-checked":{"--Switch-trackBackground":"#4ade80"},
                        "&.Mui-checked:hover":{"--Switch-trackBackground":"#388E3C"}
                      }}
                    />
                  </div>
                  ...
                  <div className={`text-center flex items-center justify-center gap-3 font-medium ${theme ? "text-black" : "text-white"}`}>
                    <div
                      ...
                      onClick={(e)=>{e.stopPropagation(); handleProductView(data.id)}}
                    >
