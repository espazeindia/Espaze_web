          ...
          if (res?.success === true) {
            ...
            const transformedCategories = await Promise.all(
              (category || []).map(async (cat) => {
                ...
                const subcategories =
                  subRes?.success === true
                    ? (subRes.data?.sub_category || []).map((sub) => ({
                        id: sub.id,
    ...
    return (
      <div
        className={`w-full p-4 border-r flex flex-col ${
          theme ? "bg-white border-gray-200" : "bg-neutral-950 border-neutral-700"
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <button
              className={`p-2 rounded ${theme ? "hover:bg-gray-100" : "hover:bg-neutral-700"}`}
            >
        ...
        <div className={`rounded-lg border ${theme ? "border-gray-200" : "border-neutral-700"}`}>
          {/* Table Header */}
          <div
            className={`grid border-b px-3 text-sm py-2 font-semibold grid-cols-[1fr_8fr_1fr] ${
          ...
          {categories.length > 0 ? (
            categories.map((cat) => (
              <div
                ...
                className={`grid grid-cols-[1fr_8fr_1fr] items-center px-4 py-2 border-b cursor-pointer transition ${
              ...
              >
                ...
                <span
                  className={`text-sm font-semibold truncate whitespace-nowrap ${
                    theme ? "text-gray-700" : "text-neutral-300"
                  }`}
                >
                  {cat.name}
                </span>
