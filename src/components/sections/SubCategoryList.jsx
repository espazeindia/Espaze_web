...
const SubCategoryView = React.memo(
  ({ selectedCategory, selectedSubcategoryNames, onSelectionChange, saving, theme }) => {
    ...
    useEffect(() => {
      const getSubcategories = async () => {
        ...
        try {
          setLoadingSubs(true);
          const res = await CategoryServices.FetchSubcategory(
            limit,
            page,
            debounce,
            selectedCategory.id
    ...
    const toggleSubcategoryCheckbox = useCallback(
      (subName) => {
    ...
    return (
      <div
        className={`flex flex-col p-4 h-full overflow-hidden ${
          theme ? "bg-white" : "bg-transparent"
        }`}
      >
        ...
        <div className="relative w-full mb-3">
          <Search
            className={`absolute left-3 top-2.5 ${theme ? "text-gray-400" : "text-neutral-400"}`}
            size={18}
          />
          <input
            type="text"
            placeholder="Search subcategories..."
            className={`w-full pl-10 pr-4 py-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-green-300 ${
              theme
                ? "border-gray-300 text-gray-600 placeholder-gray-400 bg-white"
                : "border-neutral-600 text-neutral-300 placeholder-neutral-500 bg-zinc-800"
            }`}
            value={subSearch}
            onChange={(e) => {
        ...
        <div className={`rounded-lg border ${theme ? "border-gray-200" : "border-neutral-700"}`}>
          <div
            className={`grid border-b px-3 text-sm py-2 font-semibold grid-cols-[1fr_8fr_1.5fr] ${
              theme ? "text-[#4110a2] border-gray-200" : "text-[#b898fa] border-neutral-700"
            }`}
          >
            <div>Image</div>
          ...
          {subcategories.length > 0 ? (
            <div>
              {subcategories.map((sub) => {
                ...
                return (
                  <div
                    key={sub.id}
                    className={`grid grid-cols-[1fr_8fr_1.5fr] items-center px-4 py-2 border-b ${
                      theme ? "border-gray-100" : "border-neutral-800"
                    } ${
                      isAlreadyAdded
                        ? theme
                          ? "bg-gray-100"
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-sm flex items-center justify-center text-xs ${
                        theme ? "bg-gray-200 text-gray-500" : "bg-neutral-700 text-neutral-400"
                      }`}
                    >
                      Img
                    </div>
                    <div
                      className={`ml-3 flex-1 text-sm font-medium ${
                        theme ? "text-gray-800" : "text-neutral-300"
                      }`}
                    >
                      {sub.name}
                    </div>
            ...
            <div className={`p-4 ${theme ? "text-gray-400" : "text-neutral-400"}`}>
